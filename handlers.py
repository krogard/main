...
# CallbackData для работы с покупкой товаров
...
async def show_items(message: Message):
    ...
    for num, item in enumerate(all_items):
        ...
        markup = InlineKeyboardMarkup(
            ...
            [
                [
                    # Кнопка "купить" и передача ее айди в функцию создания коллбека
                    InlineKeyboardButton(text=_("Купить"), callback_data=buy_item.new(item_id=item.id))
                ],
            ]
        )

        # Отправка фото товара с подписью и кнопкой "купить"
        ...
        # Задержка, чтобы не упереться в лимиты
...
async def buying_item(call: CallbackQuery, callback_data: dict, state: FSMContext):
    ...
    # Информацию о товаре из базы данных
...
async def enter_quantity(message: Message, state: FSMContext):
    ...
    change_button = InlineKeyboardButton(
        text=_("Заново ввести количество"),
...
async def approval(call: CallbackQuery, state: FSMContext):
    ...
    await bot.send_message(chat_id=call.from_user.id,
                           text=_("Хорошо. \n"
                                  "Оплатите <b>{amount:,}</b> по методу указанному ниже и нажмите ").format(amount=purchase.amount/100))
    ################
...
async def checkout(query: PreCheckoutQuery, state: FSMContext):
    ...
    if success:
        ...
        await bot.send_message(query.from_user.id, _("Покупка не подтверждена, попробуйте позже..."))
